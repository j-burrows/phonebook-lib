/*-- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
 |  File:       DCountry.cs
 |  Purpose:    Defines what a sovereign nation is and how to be valid in a database.
...
namespace PhoneBookLib.Data.Entities{
    public class DCountry : BCountry, IDataUnit{
        public int key { 
        ...
        public void InitFromRow(DataRow row) {
            //Given datarow is parsed into a country.
            Country_ID = row["Country_ID"].ToInt();
            Long_Name = row["Long_Name"].ToStr();
            Short_Name = row["Short_Name"].ToStr();

            states = RepositoryFactory.Instance.Construct<DState>(Country_ID);
        }

        public override bool Equivilant(IBusinessUnit comparing){
            return this.MatchingKeyAndType<DCountry>(comparing);
        }
